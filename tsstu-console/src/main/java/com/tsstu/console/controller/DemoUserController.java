...
public class DemoUserController extends BaseController {
	...
	public ModelAndView index(Pager<DemoUser> pager) {
		ModelAndView mv = new ModelAndView("demo/demo_user_index");
		super.initPageData(pager);
		demoUserService.getList(pager);
	...
	public ModelAndView toAdd() {
		ModelAndView mv = new ModelAndView("demo/demo_user_edit");
		return mv;
	}
	...
	public Result add(DemoUser demoUser) {
		demoUser.setRegister_ip(getClientIP());
		demoUser.setRegister_time(new Date());
		demoUser.setLast_login_ip(getClientIP());
		demoUser.setLast_login_time(new Date());
		int effectRow = demoUserService.add(demoUser);
		if (effectRow > 0) {
			return success();
		}
		return error();
	...
	public Result edit(DemoUser demoUser) {
		int effectRow = demoUserService.update(demoUser);
		if (effectRow > 0) {
			return success();
		}
		return error();
	...
	public Result del(Integer id) {
		int effectRow = demoUserService.delete(id);
		if (effectRow > 0) {
			return success();
		} 
		return error();
	...
	public Result del_batch(Integer[] ids) {
		int effectRow = demoUserService.deleteBatch(ids);
		if (effectRow > 0) {
			return success();
		} 
		return error();
	...
	public ModelAndView exportExcel(Pager<DemoUser> pager){
		pager.putData("pageSize", 10000000);
		this.index(pager);
		List<DemoUser> list = pager.getResult();
