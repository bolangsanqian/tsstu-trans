...
pickerDateRange.prototype.init = function(isCompare) {
    ...
    if (this.mOpts.magicSelect && this.mOpts.theme == 'ta') {
        ...
        for (var i = 0; i < this.mOpts.calendars; i++) {
            ...
            if (this.mOpts.theme == 'ta') {
				if (this.mOpts.fromToday) {
					td = this.fillDate(endDate.getFullYear(), endDate.getMonth()+1, i);
				} else {
...
pickerDateRange.prototype.addCSS = function(isCompare, specialClass) {
    ...
    // 获取4个日期
	// 默认为当天日期 ，如果不传递startDate 和 endDate
	var start = $('#' + this.startDateId).val();
	var end =  $('#' + this.endDateId).val();

	if (start == '')  {
		start = new Date();
		$('#' + this.startDateId).val( start.getFullYear() + '-' + (start.getMonth() + 1) + '-' + start.getDate());
	}
		
	if (end == '') {
		end = new Date();
		$('#' + this.endDateId).val(end.getFullYear() + '-' + (end.getMonth() + 1) + '-' + end.getDate());
	}
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    if (isTaTheme) {
        ...
        if (this.mOpts.magicSelect) {
			...
			if (this.mOpts.fromToday && month == 12) {
