	...
	NavTab.prototype = {
		...
		_event: function(){
			...
			this.$element.on('click',".close-icon",function(){
			   self.closeTab($(this).closest('.nav-tabs-item').attr('menu_id'));
			   return false;
			});
		...
		_createOneTabHtml: function(data){
			if(!data) return;
			var id = this.toValidId(data.menu_id),
				html;
			if(data.name === 'home'){
				html = '<li class="nav-tabs-item home" menu_id="' + data.menu_id + '" name="'+ data.name +'" label="'+ data.label +'" id="_tab_'+ id +'" data-url="'+ data.url +'"><i></i><span>'+ data.label +'</span></li>';
			}else{
				html = '<li class="nav-tabs-item" menu_id="' + data.menu_id + '" name="'+ data.name +'" label="'+ data.label +'" id="_tab_'+ id +'" data-url="'+ data.url +'"><span>'+ data.label +'</span><a href="javascript:;" title="关闭" class="close-icon"></a></li>';
			}
		...
		_createIframeContent: function(data){
			...
			contens.find(".menuson").hide();
			$leftMenu.addClass('active');
			$leftMenu.parent().show();
		...
		closeTab: function(menu_id){
			var id = this.toValidId(menu_id),
				$tab = $('#_tab_' + id),
				$page = $('#_page_'+ id),
			...
			for(var i = 0, len = this.opendTab.length; i < len; i++){
				if(menu_id === this.opendTab[i].menu_id){
		...
		closeOther: function(){
			...
			for(var i = 0,len = opendTab.length; i < len; i++){
				...
				if(data.name !== 'home' && data.name !== name){
					var menu_id = $('#_tab_' + id).attr('menu_id');
					this.closeTab(menu_id);
				}
			}
		...
		reloadCurrentFrame: function(){
			...
			$('#_page_' + currentId).find('iframe')[0].contentWindow.location.href = this.url;
		},
