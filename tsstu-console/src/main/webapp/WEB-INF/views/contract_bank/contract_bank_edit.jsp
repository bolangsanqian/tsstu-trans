    ...
    <div class="formbody">
    	<form id="form1">
		    ...
		    <table class="form_table">
				...
				<tr>
					<td class="right"><span class="must">*</span>银行id：</td>
					<td>
						<div class="vocation">
						    <select id="bank_id" name="bank_id" class="select3">
						    	<option value="">请选择</option>
							    <c:forEach items="${bankList }" var="dict" varStatus="dict_status">
									<option value="${dict.item_value}" <c:if test="${model.bank_id eq dict.item_value}">selected</c:if>>${dict.item_name}</option>
								</c:forEach>
						    </select>
					    </div>
					 </td>
				...
				<tr>
					<td class="right"><span class="must">*</span>签约状态：</td>
					<td>
						<div class="vocation">
						    <select id="status" name="status" class="select3">
						    	<option value="">请选择</option>
							    <c:forEach items="${D_CONTRACT_STATUS }" var="dict" varStatus="dict_status">
...
<script type="text/javascript">
	...
	function save() {
		...
		if ($.trim($("#id").val())=="") {
			add("${basePath}contract_bank/add");
		} else {
			edit("${basePath}contract_bank/edit");
		}
	}
	...
	function inputCheck() {
		...
		if ($.trim($("#bank_id").val())=="" && $.trim($("#bank_id").val()) == "") {
			$("#bank_id").tips("请选择银行id").focus();
			return;
		}
