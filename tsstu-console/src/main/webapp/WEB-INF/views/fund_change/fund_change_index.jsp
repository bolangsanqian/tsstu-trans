...
<body  style="min-width: 900px;">
	...
	<div class="place">
	    ...
	    <ul class="placeul">
		    ...
		    <li><a href="javascript:void();">加减币管理</a></li>
	    </ul>
    </div>
	    
    <div class="rightinfo">
    	<form id="pageForm" action="fund_change/index" method="post">
	    	<div class="formtitle1">
		    	<span>查询条件</span>
	    	</div>
	    	<ul class="seachform1">
			    <li><label>模糊查询：</label><input id="keywords" name="keywords" type="text" class="scinput1" value="${pageInfo.pageData.keywords }" placeholder="这里输入客户昵称、手机号码、加减币流水号、申请原因、申请时间、状态、审核备注" title="客户昵称、手机号码、加减币流水号、申请原因、申请时间、状态、审核备注"/></li>
			    ...
			    <li>
			    	...
			    	<div class="vocation">
			    		<select id="status" name="status" class="select3">
						    ...
						    <c:forEach items="${D_FUND_CHANGE_TYPE }" var="item" varStatus="status">
			    	...
			    	<div class="vocation">
			    		<select id="operation_member_id" name="operation_member_id" class="select3" onchange="getMemberList()">
						    ...
						    <c:forEach items="${DICT_OPERATION }" var="item" varStatus="status">
								<option value="${item.item_value}" <c:if test="${pageInfo.pageData.operation_member_id eq item.item_value}">selected</c:if>>${item.item_name}</option>
							</c:forEach>
					    </select>
			    	</div>
			    </li>
			    	...
			    	<div class="vocation">
			    		<select id="agent_member_id" name="agent_member_id" class="select3">
						    ...
						    <c:forEach items="${DICT_AGENT_MEMBER }" var="item" varStatus="status">
								<option value="${item.item_value}" <c:if test="${pageInfo.pageData.agent_member_id eq item.item_value}">selected</c:if>>${item.item_name}</option>
							</c:forEach>
					    </select>
			    	</div>
			    </li>
		    ...
		    <div class="tools">
		        <ul class="toolbar1">
		        	<tsstu:hasPermission sign="fund_change:add">
			        <li class="click" onclick="toAdd('${basePath }fund_change/add', 350, 310)"><span><img src="static/images/icon/icon_query.png" /></span>查询</li>
			        </tsstu:hasPermission>
		    		<tsstu:hasPermission sign="fund_change:add">
			        <li class="click" onclick="toAdd('${basePath }fund_change/add', 350, 310)"><span><img src="static/images/icon/icon_add.png" /></span>添加</li>
			        </tsstu:hasPermission>
			        <tsstu:hasPermission sign="fund_change:del">
			        <li class="click" onclick="delBatch('${basePath }fund_change/delBatch')"><span><img src="static/images/icon/icon_del.png" /></span>删除</li>
			        </tsstu:hasPermission>
			        <tsstu:hasPermission sign="fund_change:review">
			        <li class="click" onclick="review('${basePath }fund_change/review')"><span><img src="static/images/icon/icon_add.png" /></span>审批</li>
			        </tsstu:hasPermission>
			        <tsstu:hasPermission sign="fund_change:exportExcel">
			        <li class="click" onclick="exportExcel('${basePath }fund_change/exportExcel')"><span><img src="static/images/icon/icon_export.png" /></span>导出</li>
			        </tsstu:hasPermission>
		        </ul>
		    </div>
		    ...
		    <table class="tablelist">
		        ...
		        <c:choose>
		        	<c:when test="${not empty pageInfo.result}">
		        		<tsstu:hasPermission sign="fund_change:query">
			        	<c:forEach items="${pageInfo.result }" var="model" varStatus="status">
			        		<tr>
    							...
    							<td>
    								<tsstu:hasPermission sign="member_fund_change:query"><a class="link-a" href="javascript:void(0);" onclick="toDetail('${basePath}/fund_change/detail?id=${model.id }')">${model.flow_no }</a></tsstu:hasPermission>
						        ...
						        <td>
						        	<c:if test="${model.status eq 0 }">
							        	<tsstu:hasPermission sign="fund_change:del">
							        	<a href="javascript:del('${basePath }fund_change/del?id=${model.id }')" class="tablelink"> 删除</a>
							        	</tsstu:hasPermission>
							        	<tsstu:hasPermission sign="fund_change:edit">
							        	<a href="javascript:toEdit('${basePath }fund_change/edit?id=${model.id }', 600, 310)" class="tablelink"> 修改</a>
							        	</tsstu:hasPermission>
						        	</c:if>
						        </td>
					        </tr> 
			        	...
			        	<tsstu:notPermission sign="fund_change:query">
		        		<tr>
		        	...
		        	<c:otherwise>
			        	<tr>
							<td colspan="100" align="center">
								<tsstu:notPermission sign="fund_change:query">您无权查看</tsstu:notPermission>
								<tsstu:hasPermission sign="fund_change:query">没有相关数据</tsstu:hasPermission>
							</td>
						</tr>
		        	</c:otherwise>
		        </c:choose>
...
<script type="text/javascript">
	...
	function review(url) {
		...
		diag.Height = 260;
		diag.Title = "审批";
		diag.URL = "${basePath}fund_change/review?ids=" + ids;
		diag.show();
	}
</script>
</body>
