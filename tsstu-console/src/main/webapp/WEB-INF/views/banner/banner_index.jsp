...
<body  style="min-width: 900px;">
	...
	<div class="place">
	    ...
	    <ul class="placeul">
		    ...
		    <li><a href="javascript:void();">轮播图片</a></li>
	    </ul>
    </div>
	    
    <div class="rightinfo">
    	<form id="pageForm" action="banner/index" method="post">
	    	<div class="formtitle1">
		    	<span>查询条件</span>
	    	</div>
	    	<ul class="seachform1">
			    <li><label>模糊查询：</label><input id="keywords" name="keywords" type="text" class="scinput1" value="${pageInfo.pageData.keywords }" placeholder="这里输入标题、图片地址、跳转地址、交易模式、状态、排序、" title="交易模式、状态、"/></li>
			    <li><label>标题：</label><input id="title" name="title" type="text" class="scinput1" value="${pageInfo.pageData.title }" placeholder="这里输入标题" title="标题"/></li>
			    <li><label>图片地址：</label><input id="image_url" name="image_url" type="text" class="scinput1" value="${pageInfo.pageData.image_url }" placeholder="这里输入图片地址" title="图片地址"/></li>
			    <li><label>跳转地址：</label><input id="jump_url" name="jump_url" type="text" class="scinput1" value="${pageInfo.pageData.jump_url }" placeholder="这里输入跳转地址" title="跳转地址"/></li>
			    <li>
			    	<label>交易模式：</label>
			    	<div class="vocation">
			    ...
			    <li>
			    	...
			    	<div class="vocation">
			    		<select id="status" name="status" class="select3">
					    	<option value="">请选择</option>
						    <c:forEach items="${D_TRUE_FALSE }" var="item" varStatus="status">
								<option value="${item.item_value}" <c:if test="${pageInfo.pageData.status eq item.item_value}">selected</c:if>>${item.item_name}</option>
							</c:forEach>
					    </select>
			    	</div>
			    </li>
			    <li><label>排序：</label><input id="sort" name="sort" type="text" class="scinput1" value="${pageInfo.pageData.sort }" placeholder="这里输入排序" title="排序"/></li>
	    	</ul>
			<!-- 操作按钮 -->
		    <div class="tools">
		        <ul class="toolbar1">
		    		<tsstu:hasPermission sign="banner:query">
		    		<li class="click" onclick="search()"><span><img src="static/images/icon/icon_query.png" /></span>查询</li>
		    		</tsstu:hasPermission>
		    		<tsstu:hasPermission sign="banner:add">
			        <li class="click" onclick="toAdd('${basePath }banner/add', 420, 440)"><span><img src="static/images/icon/icon_add.png" /></span>添加</li>
			        </tsstu:hasPermission>
			        <tsstu:hasPermission sign="banner:del">
			        <li class="click" onclick="delBatch('${basePath }banner/delBatch')"><span><img src="static/images/icon/icon_del.png" /></span>删除</li>
			        </tsstu:hasPermission>
			        <tsstu:hasPermission sign="banner:exportExcel">
			        <li class="click" onclick="exportExcel('${basePath }banner/exportExcel')"><span><img src="static/images/icon/icon_export.png" /></span>导出</li>
			        </tsstu:hasPermission>
		        </ul>
		    </div>
		        	...
		        	<c:when test="${not empty pageInfo.result}">
			        	...
			        	<c:forEach items="${pageInfo.result }" var="model" varStatus="status">
			        		<tr>
						        ...
						        <td>
						        	<tsstu:hasPermission sign="banner:del">
						        	<a href="javascript:del('${basePath }banner/del?id=${model.id }')" class="tablelink"> 删除</a>
						        	</tsstu:hasPermission>
						        	<tsstu:hasPermission sign="banner:edit">
						        	<a href="javascript:toEdit('${basePath }banner/edit?id=${model.id }', 420, 440)" class="tablelink"> 修改</a>
						        	</tsstu:hasPermission>
						        </td>
					        </tr> 
			        	</c:forEach>
			        	</tsstu:hasPermission>
			        	<tsstu:notPermission sign="banner:query">
		        		<tr>
		        	...
		        	<c:otherwise>
			        	<tr>
							<td colspan="100" align="center">
								<tsstu:notPermission sign="banner:query">您无权查看</tsstu:notPermission>
								<tsstu:hasPermission sign="banner:query">没有相关数据</tsstu:hasPermission>
							</td>
						</tr>
		        	</c:otherwise>
		        </c:choose>
