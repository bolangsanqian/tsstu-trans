...
<body  style="min-width: 900px;">
	...
	<div class="place">
	    ...
	    <ul class="placeul">
		    ...
		    <li><a href="javascript:void();">平仓订单</a></li>
	    </ul>
    </div>
	    
    <div class="rightinfo">
    	<form id="pageForm" action="cycle_order/close_index" method="post">
		    ...
		    <div class="tools">
		        <ul class="toolbar1">
		    		<tsstu:hasPermission sign="cycle_order:close_query">
		    		<li class="click" onclick="search()"><span><img src="static/images/icon/icon_query.png" /></span>查询</li>
		    		</tsstu:hasPermission>
			        <tsstu:hasPermission sign="cycle_order:close_exportExcel">
		        	...
		        	<c:when test="${not empty pageInfo.result}">
			        	...
			        	<c:forEach items="${pageInfo.result }" var="model" varStatus="status">
			        		<tr>
    							...
    							<td>
    								<tsstu:hasPermission sign="cycle_order:close_query"><a class="link-a" href="javascript:void(0);" onclick="toDetail('${basePath}/cycle_order/close_detail?id=${model.id }')">${model.flow_no }</a></tsstu:hasPermission>
	    							<tsstu:notPermission sign="cycle_order:close_query">${model.flow_no }</tsstu:notPermission>
	    						</td>
    							<td>${model.mobile }</td>
    							<td>${model.nick_name }</td>
    							<td>${model.product_name }</td>
    							<td class="right">${model.amount }</td>
    							<td class="right" style="font-weight: bold; <c:if test="${model.yk_amount gt 0 }">color: red</c:if>">
    								${model.yk_amount }
    							</td>
    							...
    							<td><fmt:formatNumber value="${model.close_price}" type="currency" groupingUsed="false" minFractionDigits="0" maxFractionDigits="5" currencySymbol=""/></td>
								<td><fmt:formatDate value="${model.open_time }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
								<td><fmt:formatDate value="${model.close_time }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					        </tr> 
			        	</c:forEach>
			        	</tsstu:hasPermission>
			        	<tsstu:notPermission sign="cycle_order:close_query">
		        		<tr>
		        	...
		        	<c:otherwise>
			        	<tr>
							<td colspan="100" align="center">
								<tsstu:notPermission sign="cycle_order:close_query">您无权查看</tsstu:notPermission>
								<tsstu:hasPermission sign="cycle_order:close_query">没有相关数据</tsstu:hasPermission>
							</td>
						</tr>
		        	</c:otherwise>
		        </c:choose>
