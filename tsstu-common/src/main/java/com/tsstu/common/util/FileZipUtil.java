...
public class FileZipUtil {
	...
	private static void zip(ZipOutputStream out, File f, String base) throws Exception {
		if (f.isDirectory()) {
			...
			for (int i = 0; null!=fl && i < fl.length; i++) {
	 ...
	 public static void main(String [] temp){       
		 try {           
			 zip("E:\\workspace\\.metadata\\.plugins\\org.eclipse.wst.server.core\\tmp1\\wtpwebapps\\tsstu-console\\temp\\code","E:\\tes1t.zip");//你要压缩的文件夹      和  压缩后的文件 
			 }catch (Exception ex) {       
				 ex.printStackTrace();    
			 }   
		}
